...
function LoginForm() {
  ...
  const handleAuthenticate = async (event) => {
    ...
    try {
      ...
      console.log(
        "Received response from /auth ONLY RESPONSE WITHOUT .DATA:",
        response
      );
      ...
      if (authUrl) {
        ...
        console.error(
          "Authentication URL not found in response",
          response.data
        );
      }
    } catch (error) {
    ...
    <div>
      ...
      <form className="formPanel" onSubmit={login}>
        ...
        <div>
          ...
          <button className="btn" type="button" onClick={handleAuthenticate}>
            Authenticate via Schwab
          </button>
        </div>
      </form>
    </div>
