...
function AccountOverview({ user }) {
  ...
  const incomingChartData = useMemo(() => {
    ...
    const dates = chartData.quotes.map((quote) =>
      new Date(quote.date).toLocaleDateString()
    ); // take dates from inc chart data (quote.date), creates an new Date instance object with each, then maps them to new date array and formats them nicely based on locale convetions (i.e, tolocale part)
    ...
    const scaleIt =
      netLiquidatingValue / chartData.quotes[chartData.quotes.length - 1].close;
